...
| 3 | 이미 정렬된 이웃 카메라 | `x`, `y`, `rot_z` | 2단계가 불가능한 카메라용. 기준의 오차가 전달됨 |

**3단계가 필요한 이유 — 라이다는 아래를 보지 못합니다.** 실측 점군(20,000점)의 최저 높이가 모든 거리 대역에서 `z = 0.96 m` 입니다. 라이다 원점이 `z = 0.910 m` 이므로 아래로 향하는 빔이 없습니다.
...
반면 내림각이 큰 카메라가 벽에서 볼 수 있는 높이는 이렇습니다.

| 카메라 | 벽에서 0.3 m | 0.5 m | 0.8 m | 1.2 m |
| --- | --- | --- | --- | --- |
| `fdown` (54°) | 0.60 m | 0.50 m | 0.36 m | 0.18 m |
| `rear` (50°) | 0.69 m | 0.61 m | 0.50 m | 0.34 m |

**어떤 거리에서도 0.96 m 에 닿지 못합니다.** 즉 `fdown`·`rear` 의 `x`·`y`·`rot_z` 는 라이다로 맞출 방법이 원리적으로 없습니다. 접근 거리를 좁혀도 해결되지 않습니다.

카메라별 실측 겹침:

```
front   카메라 0.06~3.18 m  vs  라이다 1.16~3.44 m   겹침 2.02 m
right   카메라 0.13~1.40 m  vs  라이다 0.99~2.09 m   겹침 0.41 m
left    카메라 0.12~1.08 m  vs  라이다 0.98~1.83 m   겹침 0.10 m  ← 얇음
fdown   카메라 0.74 m       vs  라이다 0점            겹침 없음
rear    카메라 0.09~0.56 m  vs  라이다 0점            겹침 없음
```

**잘못된 매칭은 일어나지 않습니다.** 대응점 최대 거리(40~100 mm), 시야 겹침 bbox 선별, 대응점 하한 80개 때문에 0.4 m 이상 떨어진 점끼리는 짝이 지어지지 않습니다. 겹치지 않는 카메라는 2단계가 `대응점 부족 (0개 < 80)` 으로 거부되고 값이 유지됩니다.

세 단계 모두 **독립으로 판정**합니다. 라이다 겹침이 얇아 2단계가 거부돼도 1단계만 적용되며, 그것만으로도 바닥이 그라운드에서 가라앉는 문제는 사라집니다. 보고도 단계별로 따로 나옵니다.
...
2단계가 거부된 카메라는, **이번 회차에 2단계를 통과한 카메라**와 **이전 회차에 정렬해 잠근 카메라**를 기준으로 붙입니다. 기준 점군에서 수직 면을 뽑고, 정렬 대상 카메라의 점군을 소스로 `x`·`y`·`rot_z` 만 풉니다. 바닥(`z < 0.06 m`)은 제외합니다 — 이 세 축을 구속하지 못하면서 대응점만 늘립니다.

잠긴 카메라는 계산에서 빠지지만 요청에는 `reference: true` 로 함께 전달되어 기준으로 쓰입니다. 보고에 `잠김 — 계산 제외, 3단계의 기준으로만 사용` 으로 표시됩니다.

| 거부 조건 | 기준 |
| --- | --- |
| 기준 카메라 | 2단계를 통과했거나 잠긴 카메라가 최소 1대 |
| 겹치는 영역 점 수 | ≥ 300 |
| 겹치는 영역의 수직 면 | 있어야 함 (바닥만 겹치면 세 축 모두 미끄러짐) |
| 나머지 | 2단계와 동일 (`fitness`, `rmse`, 보정량, 축별 불확실도) |

**주의: 기준의 오차가 그대로 전달됩니다.** 그래서 라이다로 직접 맞춘 카메라만 기준이 됩니다. 3단계로 맞춘 카메라를 다시 기준으로 쓰는 연쇄는 하지 않습니다.

실사용상 `fdown`·`rear` 는 이 경로로만 맞출 수 있습니다. `front` 가 바닥부터 3 m 높이까지 보므로, 같은 벽 밑동을 `fdown`·`rear` 와 함께 보도록 세우면 기준이 됩니다.
...
| `lidar_blind` | 라이다와 시야 대역이 겹치지 않음 (`fdown`, `rear`) | 접근·회전 무의미. 이미 정렬된 카메라와 같은 벽을 함께 보게 배치 → 3단계 |
